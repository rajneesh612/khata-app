...
const seedCatalog = (database: Database.Database): void => {
  ...
  database.transaction(() => {
    ...
    (database.prepare("SELECT id, name FROM categories").all() as Array<
      Category
    >).forEach((category) => categoryMap.set(category.name, category.id));
...
export const getLedgerEntries = (customerId: number): LedgerEntry[] => {
  ...
  return stmt.all(customerId) as LedgerEntry[];
};
...
export const addCategory = (payload: { name: string }): Category => {
  const database = getDb();
  const name = payload.name.trim();
  if (!name) {
...
export const getCustomerSummary = (customerId: number): {
  balance: number;
  totalDebit: number;
  totalCredit: number;
} => {
...
export const getLedgerAging = (customerId: number): {
  current: number;
  days30: number;
  days60: number;
  days90: number;
