...
function App() {
  ...
  const loadCustomers = async () => {
    try {
      ...
      const summaryPromises = data.map(async (customer) => {
        try {
          const sResp = await api.get(`/summary/${customer.id}`)
  ...
  const loadLedger = async (customerId: number) => {
    try {
      const [entriesResp, summaryResp, agingResp] = await Promise.all([
        api.get(`/ledger/${customerId}`),
        api.get(`/summary/${customerId}`),
        api.get(`/aging/${customerId}`),
      ])
  ...
  const handleCustomersCsvDownload = async () => {
    try {
      ...
      await downloadFile('/reports/customers/csv', 'customers-summary.csv', 1)
    } catch (error: any) {
      alert(error.message)
    } finally {
  ...
  const handleCustomerLedgerCsvDownload = async () => {
    ...
    try {
      ...
      await downloadFile(
        `/reports/ledger/csv?customerId=${selectedCustomerId}`,
  ...
  return (
    <Router>
      <Routes>
        <Route 
          path="/login" 
          element={!shop ? <Auth onLogin={(s) => { setShop(s); window.location.href = '/'; }} /> : <Navigate to="/" />} 
        />
        <Route 
          path="/" 
          element={shop ? (
            <div className="app-container">
              <header className="app-header">
                <div className="shop-info">
                  <h1>{shop.name}</h1>
                  <span className="owner-badge">{shop.owner}</span>
                </div>
                ...
                <section className="panel">
                  ...
                  {selectedCustomer && aging ? (
                    ...
                    <div className="empty-state-card inventory-empty-state">
                      <h3>Welcome!</h3>
                      <p>Select a customer to view their ledger and balance details.</p>
                    </div>
                  )}
                </section>
